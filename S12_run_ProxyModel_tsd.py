...
variable_parameters = {"talent_sd": [0, 0.25, 0.5, 1, 2, 3], "seed": range(10)}
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        runcolor = np.zeros([len(talent_sd_data.AgentID.unique()),
                             len(talent_sd_data.RunId.unique())])
        for index, iteration in enumerate(talent_sd_data.RunId.unique()):
            for Agent in talent_sd_data.AgentID.unique():
                proxy[Agent][index] = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        talent_sd = modeldata[modeldata.RunId == run].talent_sd.iloc[0]
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(talent_sd_data.RunId.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == step)].Practice
...
#                for Agent in talent_sd_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = talent_sd_data[(talent_sd_data.RunId == iteration) & (talent_sd_data.Step == step)].Talent.iloc[Agent]
