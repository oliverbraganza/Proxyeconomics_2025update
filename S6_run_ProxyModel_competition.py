...
selection_pressure (p): probability of loser death (0 to 1) 
goal_scale (g): scaling factor of psychological goal valuation
goal_angle (gamma): angle between proxy and goal (0° to 180°) defining practice space
"""


finalStep = 500
variable_parameters = {"competition": np.linspace(0.1, 0.9, 9) , "seed": range(3) }
parameters = {#"data_collect_interval": 1,
              ...
              "practice_mutation_rate": np.pi/45,
              "angle_agency": 0,
              "max_steps" : finalStep}
...
data_collect_interval = 1#parameters["data_collect_interval"]
...
def showModel(modeldata):
    ...
    ax1.scatter(modeldata.competition, modeldata.mean_goal_value,
                alpha=0.5, color=low_col, marker=".", label='_nolegend_')
    ax1.scatter(modeldata.competition, modeldata.mean_proxy_value,
                alpha=0.5, color=high_col, marker=".", label='_nolegend_')
...
def showAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        runcolor = np.zeros([len(competition_data.AgentID.unique()),
                             len(competition_data.RunId.unique())])
        for index, iteration in enumerate(competition_data.RunId.unique()):
            for Agent in competition_data.AgentID.unique():
                proxy[Agent][index] = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == visualizeStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == visualizeStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == visualizeStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        competition = modeldata[modeldata.RunId == run].competition.iloc[0]
...
def showTalentDynamics(modeldata):
    """ Talent Dynamics for particular competition """

    columns = 4
    f1, ax_array = plt.subplots(1, columns, figsize=(8, 1.2), sharey=True)
    f1.subplots_adjust(hspace=.3, wspace=.3, left=0.1, right=0.9)
    for i, ax in enumerate(ax_array):
        if i == 0:
...
#        print(competition/np.pi * 180)
        competition_data = modeldata[modeldata.competition == competition]
        xVals = competition_data.Step.unique()
        yVals_raw = competition_data.groupby(['Step'])['mean_talent']
        talent_mean = np.empty(np.size(xVals))
        talent_SEM = np.empty(np.size(xVals))
        i = 0
        for name, group in yVals_raw:
            talent_mean[i] = np.mean(group)
            talent_SEM[i] = np.mean(group)/np.sqrt(parameters['numAgents'])
            i += 1

        ax.plot(xVals, talent_mean, c='k')
        ax.fill_between(xVals, talent_mean - talent_SEM,
                        talent_mean + talent_SEM, alpha=0.2, color="k")
        ax.plot([0, np.max(xVals)],
                [0, 0],
                c='grey', ls='--', lw=0.5)
        ax.set_xlabel('step')
    ...
    for i, ax in enumerate(ax_array):
        ...
        for index, iteration in enumerate(competition_data.RunId.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == step)].Practice
...
#                for Agent in competition_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # competition_data[(competition_data.RunId == iteration) & (competition_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # competition_data[(competition_data.RunId == iteration) & (competition_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = competition_data[(competition_data.RunId == iteration) & (competition_data.Step == step)].Talent.iloc[Agent]
