...
variable_parameters = {"goal_angle": np.linspace(0, np.pi, 9), "seed": range(10)}
...
def showModel(modeldata):
    ...
    # colormap
    cmap = plt.cm.jet
    cNorm = colors.Normalize(vmin=np.min(modeldata.goal_angle),
                             vmax=np.max(modeldata.goal_angle))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(min(modeldata.goal_angle))
    high_col = scalarMap.to_rgba(max(modeldata.goal_angle))

    # compute means & std
    xVals = modeldata.goal_angle.unique()
    Gm = modeldata.groupby(['goal_angle'])['mean_goal_value'].mean()
    Gstd = modeldata.groupby(['goal_angle'])['mean_goal_value'].std()
    Pm = modeldata.groupby(['goal_angle'])['mean_proxy_value'].mean()
    Pstd = modeldata.groupby(['goal_angle'])['mean_proxy_value'].std()

    ax1.scatter(modeldata.goal_angle, modeldata.mean_goal_value,
                alpha=0.5, c=low_col, marker=".", label='_nolegend_')
    ax1.scatter(modeldata.goal_angle, modeldata.mean_proxy_value,
    ...
    ax1.set_xlabel('goal_angle')

    utility_mean = modeldata.groupby(['goal_angle'])['mean_utility'].mean()
    utility_std = modeldata.groupby(['goal_angle'])['mean_utility'].std()
    ax2.scatter(modeldata.goal_angle, modeldata.mean_utility,
    ...
    ax2.set_xlabel('goal_angle')

    groups_m = modeldata.groupby(['goal_angle'])['mean_practice']
    practice_mean = np.zeros(len(parameters['goal_angle']))
    i = 0
    for name, group in groups_m:
        practice_mean[i] = stats.circmean(group, -np.pi, np.pi)
        i += 1
    groups_std = modeldata.groupby(['goal_angle'])['mean_practice']
    practice_std = np.zeros(len(parameters['goal_angle']))
    i = 0
    ...
    ax2a = ax2.twinx()
    ax2a.scatter(modeldata.goal_angle,
    ...
    # vector plot
    G_oc_m = modeldata.groupby(['goal_angle'])['mean_goal_oc'].mean()
    ...
    mpl.colorbar.ColorbarBase(cbar_ax, cmap=cmap, norm=cNorm,
                              orientation='vertical', label='goal_angle')
...
def showAgentDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        ax.set_xlabel('agent #')
        sp = run_data.goal_angle.iloc[0]
        ax.set_title('ga{:.2f} r{:02d}'.format(sp, run))
...
def showAgents(agentdata):
    ...
    # ga = agentdata.goal_angle.iloc[0]
    for i, ax in enumerate(ax_array):
        if i == 0:
        ...
        goal_angle = agentdata[agentdata.RunId == run].goal_angle.iloc[0]
        goal_angle_data = agentdata[agentdata.goal_angle == goal_angle]
        proxy = np.zeros([len(goal_angle_data.AgentID.unique()),
                         len(goal_angle_data.RunId.unique())])
        goal_oc = np.zeros([len(goal_angle_data.AgentID.unique()),
                            len(goal_angle_data.RunId.unique())])
        talent = np.zeros([len(goal_angle_data.AgentID.unique()),
                           len(goal_angle_data.RunId.unique())])
        runcolor = np.zeros([len(goal_angle_data.AgentID.unique()),
                             len(goal_angle_data.RunId.unique())])
        for index, iteration in enumerate(goal_angle_data.RunId.unique()):
            for Agent in goal_angle_data.AgentID.unique():
                proxy[Agent][index] = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    """ Model Dynamics for particular goal_angle """
    ...
    cNorm = colors.Normalize(vmin=np.min(modeldata.goal_angle),
                             vmax=np.max(modeldata.goal_angle))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(np.min(modeldata.goal_angle))
    high_col = scalarMap.to_rgba(np.max(modeldata.goal_angle))
    ...
    for i, ax in enumerate(ax_array):
        ...
        goal_angle = modeldata[modeldata.RunId == run].goal_angle.iloc[0]
        goal_angle_data = modeldata[modeldata.goal_angle == goal_angle]

        xVals = goal_angle_data.Step.unique()
        proxy_mean = goal_angle_data.groupby(['Step'])['mean_proxy_value'].mean()
        proxy_std = goal_angle_data.groupby(['Step'])['mean_proxy_value'].std()
        # ax.scatter(goal_angle_data.Step,
        #           goal_angle_data.mean_proxy_value,
        ...
        goal_mean = goal_angle_data.groupby(['Step'])['mean_goal_value'].mean()
        goal_std = goal_angle_data.groupby(['Step'])['mean_goal_value'].std()
        # ax.scatter(goal_angle_data.Step,
        #           goal_angle_data.mean_goal_value,
        ...
        ax.set_xlabel('step')
        # ax.set_title('c{:.1f} r{:02d}'.format(goal_angle, run))
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(modeldata.RunId)+1)/(columns-0.9))
        goal_angle = modeldata[modeldata.RunId == run].goal_angle.iloc[0]
#        print(goal_angle/np.pi * 180)
        goal_angle_data = modeldata[modeldata.goal_angle == goal_angle]
        xVals = goal_angle_data.Step.unique()
        yVals_raw = goal_angle_data.groupby(['Step'])['mean_practice']
...
def showAgentProxyDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(agentdata.RunId)+1)/(columns-0.9))
        run_data = agentdata[agentdata.RunId == run]
        # goal_angle = run_data.goal_angle.iloc[0]
        image = np.empty((len(run_data.Step.unique()),
                          len(run_data.AgentID.unique())))
...
def showSortedAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        proxy = np.zeros([len(goal_angle_data.AgentID.unique()),
                         len(goal_angle_data.RunId.unique()),
                         StepsToAverage])
        practice = np.zeros([len(goal_angle_data.AgentID.unique()),
                             len(goal_angle_data.RunId.unique()),
                             StepsToAverage])
        talent = np.zeros([len(goal_angle_data.AgentID.unique()),
                          len(goal_angle_data.RunId.unique()),
        ...
        for index, iteration in enumerate(goal_angle_data.RunId.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == step)].Practice
...
#                for Agent in goal_angle_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # goal_angle_data[(goal_angle_data.Run == iteration) & (goal_angle_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = goal_angle_data[(goal_angle_data.RunId == iteration) & (goal_angle_data.Step == step)].Talent.iloc[Agent]

        # plot goal angle
        # ax.plot([0, 45], [0, 0], c='grey', ls='--', lw=0.5)
        xVals = goal_angle_data.AgentID.unique()
