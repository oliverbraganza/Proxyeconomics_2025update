...
variable_parameters = {"selection_pressure": [0.01, 0.1, 0.2, 1], "seed": range(10)}
...
def showModel(modeldata):
    ...
    # colormap
    cmap = plt.cm.jet
    cNorm = colors.Normalize(vmin=np.min(modeldata.selection_pressure),
                             vmax=np.max(modeldata.selection_pressure))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(min(modeldata.selection_pressure))
    high_col = scalarMap.to_rgba(max(modeldata.selection_pressure))

    # compute means & std
    xVals = modeldata.selection_pressure.unique()
    Gm = modeldata.groupby(['selection_pressure'])['mean_goal_value'].mean()
    Gstd = modeldata.groupby(['selection_pressure'])['mean_goal_value'].std()
    Pm = modeldata.groupby(['selection_pressure'])['mean_proxy_value'].mean()
    Pstd = modeldata.groupby(['selection_pressure'])['mean_proxy_value'].std()

    ax1.scatter(modeldata.selection_pressure, modeldata.mean_goal_value,
                alpha=0.5, c=low_col, marker=".", label='_nolegend_')
    ax1.scatter(modeldata.selection_pressure, modeldata.mean_proxy_value,
    ...
    ax1.set_xlabel('selection_pressure')

    utility_mean = modeldata.groupby(['selection_pressure'])['mean_utility'].mean()
    utility_std = modeldata.groupby(['selection_pressure'])['mean_utility'].std()
    ax2.scatter(modeldata.selection_pressure, modeldata.mean_utility,
    ...
    ax2.set_xlabel('selection_pressure')

    groups_m = modeldata.groupby(['selection_pressure'])['mean_practice']
    practice_mean = np.zeros(len(parameters['selection_pressure']))
    i = 0
    for name, group in groups_m:
        practice_mean[i] = stats.circmean(group, -np.pi, np.pi)
        i += 1
    groups_std = modeldata.groupby(['selection_pressure'])['mean_practice']
    practice_std = np.zeros(len(parameters['selection_pressure']))
    i = 0
    for name, group in groups_std:
        practice_std[i] = stats.circstd(group, -np.pi, np.pi)
        i += 1
    practice_mean = practice_mean/np.pi*180
    # practice_std = modeldata.groupby(['selection_pressure'])['mean_practice'].std()
    practice_std = practice_std/np.pi*180
    ax2a = ax2.twinx()
    ax2a.scatter(modeldata.selection_pressure,
    ...
    ax2a.set_ylim([0, 22.5])
    ax2a.set_ylabel('mean practice (°)')

    # vector plot
    G_oc_m = modeldata.groupby(['selection_pressure'])['mean_goal_oc'].mean()
    ...
    mpl.colorbar.ColorbarBase(cbar_ax, cmap=cmap, norm=cNorm,
                              orientation='vertical', label='selection_pressure')
...
def showAgentDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        ax.set_xlabel('agent #')
        sp = run_data.selection_pressure.iloc[0]
        ax.set_title('sp{:.2f} r{:02d}'.format(sp, run))
...
def showAgents(agentdata):
    ...
    # ga = agentdata.selection_pressure.iloc[0]
    for i, ax in enumerate(ax_array):
        if i == 0:
        ...
        selection_pressure = agentdata[agentdata.RunId == run].selection_pressure.iloc[0]
        selection_pressure_data = agentdata[agentdata.selection_pressure == selection_pressure]
        proxy = np.zeros([len(selection_pressure_data.AgentID.unique()),
                         len(selection_pressure_data.RunId.unique())])
        goal_oc = np.zeros([len(selection_pressure_data.AgentID.unique()),
                        len(selection_pressure_data.RunId.unique())])
        talent = np.zeros([len(selection_pressure_data.AgentID.unique()),
                          len(selection_pressure_data.RunId.unique())])
        runcolor = np.zeros([len(selection_pressure_data.AgentID.unique()),
                             len(selection_pressure_data.RunId.unique())])
        for index, iteration in enumerate(selection_pressure_data.RunId.unique()):
            for Agent in selection_pressure_data.AgentID.unique():
                proxy[Agent][index] = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == finalStep)].Proxy.iloc[Agent]    
                goal_oc[Agent][index] = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == finalStep)].Goal_oc.iloc[Agent]
                talent[Agent][index] = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == finalStep)].Talent.iloc[Agent]
...
def showModelDynamics(modeldata):
    """ Model Dynamics for particular selection_pressure """
    ...
    cNorm = colors.Normalize(vmin=np.min(modeldata.selection_pressure),
                             vmax=np.max(modeldata.selection_pressure))
    scalarMap = cmx.ScalarMappable(norm=cNorm, cmap=cmap)
    low_col = scalarMap.to_rgba(np.min(modeldata.selection_pressure))
    high_col = scalarMap.to_rgba(np.max(modeldata.selection_pressure))
    ...
    for i, ax in enumerate(ax_array):
        ...
        selection_pressure = modeldata[modeldata.RunId == run].selection_pressure.iloc[0]
        selection_pressure_data = modeldata[modeldata.selection_pressure == selection_pressure]

        xVals = selection_pressure_data.Step.unique()
        proxy_mean = selection_pressure_data.groupby(['Step'])['mean_proxy_value'].mean()
        proxy_std = selection_pressure_data.groupby(['Step'])['mean_proxy_value'].std()
        # ax.scatter(selection_pressure_data.Step,
        #           selection_pressure_data.mean_proxy_value,
        ...
        goal_mean = selection_pressure_data.groupby(['Step'])['mean_goal_value'].mean()
        goal_std = selection_pressure_data.groupby(['Step'])['mean_goal_value'].std()
        # ax.scatter(selection_pressure_data.Step,
        #           selection_pressure_data.mean_goal_value,
        ...
        ax.set_xlabel('step')
        # ax.set_title('c{:.1f} r{:02d}'.format(selection_pressure, run))
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(modeldata.RunId)+1)/(columns-0.9))
        selection_pressure = modeldata[modeldata.RunId == run].selection_pressure.iloc[0]
#        print(selection_pressure/np.pi * 180)
        selection_pressure_data = modeldata[modeldata.selection_pressure == selection_pressure]
        xVals = selection_pressure_data.Step.unique()
        yVals_raw = selection_pressure_data.groupby(['Step'])['mean_practice']
        ...
        practice_std = practice_std / np.pi * 180
#        ax.scatter(selection_pressure_data.Step,
#                   selection_pressure_data.mean_practice,
...
def showAgentProxyDynamics(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        run = int(i * (max(agentdata.RunId)+1)/(columns-0.9))
        run_data = agentdata[agentdata.RunId == run]
        # selection_pressure = run_data.selection_pressure.iloc[0]
        image = np.empty((len(run_data.Step.unique()),
                          len(run_data.AgentID.unique())))
...
def showSortedAgents(agentdata):
    ...
    for i, ax in enumerate(ax_array):
        ...
        proxy = np.zeros([len(selection_pressure_data.AgentID.unique()),
                         len(selection_pressure_data.RunId.unique()),
                         StepsToAverage])
        practice = np.zeros([len(selection_pressure_data.AgentID.unique()),
                             len(selection_pressure_data.RunId.unique()),
                             StepsToAverage])
        talent = np.zeros([len(selection_pressure_data.AgentID.unique()),
                           len(selection_pressure_data.RunId.unique()),
        ...
        for index, iteration in enumerate(selection_pressure_data.RunId.unique()):
            for sindex, step in enumerate(Steps):
                RunRowProxy = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == step)].Proxy
                RunRowProxy = np.array(RunRowProxy)
                RunRowPractice = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == step)].Practice
...
#                for Agent in selection_pressure_data.AgentID.unique():   
                proxy[:, index, sindex] = RunRowProxy # selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == step)].Proxy.iloc[Agent]    
                practice[:, index, sindex] = RunRowPractice # selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == step)].Practice.iloc[Agent]
                    # talent[Agent][index][sindex] = selection_pressure_data[(selection_pressure_data.RunId == iteration) & (selection_pressure_data.Step == step)].Talent.iloc[Agent]

        # plot goal angle
        # ax.plot([0, 45], [0, 0], c='grey', ls='--', lw=0.5)
        xVals = selection_pressure_data.AgentID.unique()
